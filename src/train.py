...
def train(args, model: nn.Module, device, train_data, optimizer, epoch, A):
    ...
    for pos in pbar:
        ...
        if pos > 0:
            pbar.set_description(f'Epoch: {epoch}, Loss: {loss_accum / pos:.4f}')
...
if __name__ == '__main__':
    ...
    for i in range(args.fold_idx):
        ...
        early_stopping = tools.EarlyStopping(patience=args.patience, verbose=True)
        ...
        for epoch in range(1, args.epochs+1):
            ...
            if epoch>1:
                with torch.no_grad():
                    ...
                    val_labels=torch.LongTensor([data[1] for data in test_data]).to(device)
                    val_loss=np.average(crit(val_out, val_labels).detach().cpu().numpy())
            ...
            if epoch>300 and epoch%20==0 or epoch%10==0:
                acc_train, acc_test, _, _ = test(args, model, train_data, test_data)
                
            scheduler.step()
                
        model.load_state_dict(torch.load('checkpoint.pt'))
        
        acc_train, acc_test, output, label = test(args, model, train_data, test_data)
        acc_train_sum+=acc_train
        acc_test_sum+=acc_test
        
        model=GCN(args.num_layers,d,args.hidden_dim,num_classes,args.final_dropout,args.graph_pooling_type,device,A).to(device)
        
    print('Average train acc: %f,  Average test acc: %f' % (acc_train_sum / args.fold_idx, acc_test_sum / args.fold_idx))
